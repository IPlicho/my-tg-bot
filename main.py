    ...
    try:
        ...
        if arr[0] == "派单":
            if len(arr) >= 4:
                ...
                t_a = TEXT_A[lang_t]
                s = t_a["new_order_assign"].format(oid, typename, amt, profit_val, now_time)
...
# ======================== 线程启动 ========================
def start_threads():
    t1 = threading.Thread(target=refresh_virtual_orders1, daemon=True)
    t2 = threading.Thread(target=auto_clean_orders, daemon=True)
    t1.start()
    t2.start()
