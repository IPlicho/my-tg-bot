...
def now_beijing():
    ...
    return datetime.now(timezone.utc).astimezone(cst).strftime("%m-%d %H:%M")
    ...
    try:
        ...
        if c.data == "profile":
            ...
            if lang == "zh":
                final_text = f"""👤 個人中心
🆔 用戶ID：{u}
💰 餘額：{user_balance1.get(u, 0.0):.2f} USD
📌 狀態：{status}
...
💵 累計已賺利潤：{my_profit} USD"""
            else:
                final_text = f"""👤 Profile
🆔 User ID: {u}
💰 Balance: {user_balance1.get(u, 0.0):.2f} USD
📌 Status: {status}
